...
export function registerLifecycleCommands(cmd: Command): void {
      ...
      try {
        ...
        if (created.length > 0) {
          ...
          for (const pr of created)
            console.log(`  #${pr.number}  ${pr.title}  (${pr.headBranch} → ${pr.baseBranch})`)
        }
        if (updated.length > 0) {
  ...
  cmd
    ...
    .action(async (branch: string, opts: { repo?: string; dryRun?: boolean; remote: string }) => {
      ...
      try {
        const { closedPr, retargeted } = await closePR(client, {
          repo,
          branch,
          dryRun: opts.dryRun,
        })
        ...
        if (opts.dryRun) {
          ...
          if (retargeted.length > 0) {
            console.log(
              `Would re-target ${retargeted.length} child PR(s) to base "${closedPr.baseBranch}":`,
            )
        ...
        if (retargeted.length > 0) {
          console.log(
            `Re-targeted ${retargeted.length} child PR(s) to base "${closedPr.baseBranch}":`,
          )
          for (const pr of retargeted) console.log(`  #${pr.number}  ${pr.headBranch}`)
        }
      } catch (err) {
  ...
  cmd
    ...
    .option(
      "--single",
      "Merge only this PR and re-target its children (default: merge whole stack)",
    )
    .option("--dry-run", "Show what would happen without making any changes")
    .action(
      async (
        ...
        try {
          if (opts.single) {
            ...
            if (opts.dryRun) {
              ...
              if (retargeted.length > 0) {
                console.log(
                  `Would re-target ${retargeted.length} child PR(s) to "${mergedPr.baseBranch}":`,
                )
                for (const pr of retargeted) console.log(`  #${pr.number}  ${pr.headBranch}`)
              }
              return
            ...
            if (retargeted.length > 0) {
              console.log(
                `Re-targeted ${retargeted.length} child PR(s) to "${mergedPr.baseBranch}":`,
              )
              for (const pr of retargeted) {
                console.log(`  #${pr.number}  ${pr.headBranch}`)
                try {
                  setParent(pr.headBranch, pr.baseBranch, process.cwd())
                } catch {
                  /* ignore */
                }
              }
    ...
    .action(
      async (
        ...
        while (bfsQueue.length > 0) {
          const b = bfsQueue.shift() as string
          if (seen.has(b)) continue
          seen.add(b)
          if (!branchExists(b, cwd)) {
            console.warn(
              `Warning: "${b}" is in stack config but does not exist locally — skipping.`,
            )
            continue
          }
  ...
  cmd
    .command("gc")
    .description(
      "Remove stale stack config entries for branches that no longer exist locally or on the remote",
    )
    .option(
      "--remote",
      "Also prune entries with no open PR and no remote branch (requires API access)",
    )
    .option(
      "--repo <owner/repo>",
      "GitHub/GitLab repository (default: auto-detect from git remote)",
    )
    ...
    .action(
      async (opts: { remote?: boolean; repo?: string; remoteName: string; dryRun?: boolean }) => {
        const cwd = process.cwd()
        ...
        if (opts.remote) {
          ...
          console.log(
            `Checking ${entries.length} entr${entries.length === 1 ? "y" : "ies"} against remote...`,
          )
