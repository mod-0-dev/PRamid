...
export function buildBranchCommand(): Command {
  ...
  cmd
    .command("new <name>")
    .description("Create a new branch stacked on the current one and record the parent relationship")
    .action((name: string) => {
...
export function registerPushCommand(program: Command): void {
  program
    ...
    .action(async (opts: { base?: string; draft?: boolean; title?: string; repo?: string; remote: string }) => {
      const cwd = process.cwd()
        ...
        try {
          ...
          if (base) {
            ...
            if (!baseHasPr && !isTrunk) {
              console.warn(`Warning: base branch "${base}" has no open PR and is not a known trunk branch.`)
            }
          }
        } catch (err) {
      ...
      if (!base) {
        ...
        console.error(`Use --base <branch> to specify it explicitly, or create this branch with \`pramid branch new\`.`)
        process.exit(1)
      }
