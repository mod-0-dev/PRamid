...
import { cy, buildElements, highlightCurrentBranch } from "./graph";
...
function populateBranchSelect(branches: Array<{ name: string; upstream: string | null }>, current: string): void {
  // Only rebuild options when the list changes to avoid flicker
  const existing = Array.from(selectEl.options).map((o) => o.value);
  const same = existing.length === branches.length && branches.every((b, i) => b.name === existing[i]);
  if (!same) {
    selectEl.innerHTML = "";
...
export async function fetchAndRender(): Promise<void> {
  try {
    ...
    let currentBranch = "";
    if (branchRes.ok) {
      const { current, branches } = (await branchRes.json()) as { current: string; branches: Array<{ name: string; upstream: string | null }> };
